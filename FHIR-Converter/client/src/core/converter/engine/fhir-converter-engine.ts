...
export class FhirConverterEngine implements IConverterEngine {
	...
	constructor(
		...
		rootTemplate: string,
		resultFolder: string,
		engineExecCmd: string = engineConstants.DefaultEngineExecCmd,
	) {
	...
	process(dataFile: string) {
		...
		if (!fs.existsSync(dataFile)) {
			throw new ConversionError(
				localize("message.dataFileNotExists", dataFile),
			);
		}
		...
		const defaultResultFile = path.join(
			this._resultFolder,
			engineConstants.DefaultResultFile,
		);
		const rootTemplate = stringUtils.getFileNameWithoutExt(
			this._rootTemplate,
		);
		const paramList = [
			" convert",
			"-d",
			stringUtils.addQuotes(this._templateFolder),
			"-r",
			stringUtils.addQuotes(rootTemplate),
			"-n",
			stringUtils.addQuotes(dataFile),
			"-f",
			stringUtils.addQuotes(defaultResultFile),
			"-t",
		];
		...
		if (fs.existsSync(defaultResultFile)) {
			const resultMsg = JSON.parse(
				fs.readFileSync(defaultResultFile).toString(),
			);
			if (!engineUtils.checkConversionSuccess(resultMsg)) {
				throw new ConversionError(
					localize("message.noResponseFromEngine"),
				);
			}
