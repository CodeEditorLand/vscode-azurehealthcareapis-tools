...
suite("Converter Test Suite", () => {
	...
	const fhirConverterEngine = new FhirConverterEngine(
		templateFolder,
		rootTemplate,
		resultFolder
	);
	...
	test("Function getHistory - should return a list of files given a file path", async () => {
		...
		const expectedList = [
			path
				.join(historyFolder, "ADT04-28 - ADT_A01.1605684048847.json")
				.replace(/\\/g, "/"),
			path
				.join(historyFolder, "ADT04-28 - ADT_A01.1605683993535.json")
				.replace(/\\/g, "/"),
			path
				.join(historyFolder, "ADT04-28 - ADT_A01.1605683976874.json")
				.replace(/\\/g, "/"),
		];
